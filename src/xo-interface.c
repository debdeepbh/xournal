...
#define	TOOLBARDIR 0	// toolbarMain orientation: 1=left, 0=top
#define MERGETOOLBARPEN 0 // Merge toolbarPen with toolbarMain
...
create_winMain (void)
{
  ...
  GtkWidget *aboxMain; // place to attach the toolbarMain and toolbarPen
  ...
  vboxMain = gtk_vbox_new (FALSE, 0);	// create a vertical box
  ...
  if(TOOLBARDIR ==1){
	  hboxSubMain = gtk_hbox_new (FALSE, 0);	//create a horizontal box
	  gtk_widget_show (hboxSubMain);
	  gtk_box_pack_start (GTK_BOX (vboxMain), hboxSubMain, TRUE, TRUE, 0);

	  aboxMain = hboxSubMain;
  } else if (TOOLBARDIR==0){
	  aboxMain = vboxMain;
  }
  ...
  gtk_box_pack_start (GTK_BOX (aboxMain), toolbarMain, FALSE, FALSE, 0);
  ...
  if (MERGETOOLBARPEN == 1) {
    toolbarPen = toolbarMain;
  } else {
    toolbarPen = gtk_toolbar_new();
    if (TOOLBARDIR == 1) {
      gtk_toolbar_set_orientation(GTK_TOOLBAR(toolbarPen),
                                  GTK_ORIENTATION_VERTICAL);
    }
    gtk_widget_show(toolbarPen);
    gtk_box_pack_start(GTK_BOX(aboxMain), toolbarPen, FALSE, FALSE, 0);
    // gtk_box_pack_end (GTK_BOX (aboxMain), toolbarPen, FALSE, FALSE, 0);
  ...
  gtk_box_pack_start (GTK_BOX (aboxMain), scrolledwindowMain, TRUE, TRUE, 0);
